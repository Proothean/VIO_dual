...
//       map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1 = cv::Mat());
    ...
    cv::Mat cur_img1, forw_img1,prev_img1;

    vector<cv::Point2f> n_pts;

    vector<cv::Point2f> cur_pts, forw_pts, prev_pts;
    vector<cv::Point2f> cur_pts1, forw_pts1, prev_pts1;

    vector<cv::Point2f> prev_un_pts, cur_un_pts;
    vector<cv::Point2f> cur_un_pts1;

    vector<cv::Point2f> pts_velocity, pts_velocity1;

    vector<int> ids, ids1;
    vector<int> track_cnt;

    map<int, cv::Point2f> cur_un_pts_map, cur_un_pts_map1;
    map<int, cv::Point2f> prev_un_pts_map,prev_un_pts_map1;
