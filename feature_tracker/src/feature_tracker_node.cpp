...
queue<geometry_msgs::PointStamped> groutruth_buf;
std::mutex m_buf;
std::mutex g_buf;
...
geometry_msgs::PointStamped groutruth_msg;

void ground_truth_callback(const geometry_msgs::PointStamped groutruth_msg)
{
    g_buf.lock();
    groutruth_buf.push(groutruth_msg);
    g_buf.unlock();
    //ROS_INFO("ground truth seq %f: [x]: %f, [y]: %f, [z]: %f", 
	     //groutruth_msg.header.stamp.toSec(), groutruth_msg.point.x, groutruth_msg.point.y, groutruth_msg.point.z);
}
...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    m_buf.unlock();
    //ROS_INFO("img seq %f: ", img_msg.get()->header.stamp.toSec());
}
...
//           trackerData.readImage(ptr->image.rowRange(0, ROW), ptr1->image.rowRange(0,ROW),img_msg->header.stamp.toSec());
          
          trackerData.trackImage(img_msg->header.stamp.toSec(),ptr->image.rowRange(0, ROW),ptr1->image.rowRange(0, ROW));
...
/*         
        ...
        if (PUB_THIS_FRAME)
        {
		    ...
		    //ROS_INFO("left id = %i",ids[j]);
		    //ROS_INFO("left publish %f, left count is: %d, point id is: %f, [u]: %f, [v]: %f", feature_points->header.stamp.toSec(), count_feature_left, ids[j], un_pts[j].x, un_pts[j].y);
		    //ROS_INFO("left u_of_point = %f, v_of_point = %f",cur_pts[j].x, cur_pts[j].y);
		    ...
		    //ROS_INFO("right publish %f, right count is: %d, point id is: %f, [u]: %f, [v]: %f", feature_points->header.stamp.toSec(), count_feature_right, ids[j], un_pts1[j].x, un_pts1[j].y);
                    //ROS_INFO("right id = %i",ids1[j]);
		    //ROS_INFO("right u_of_point = %f, v_of_point = %f",cur_pts1[j].x, cur_pts1[j].y);
...
*/
    ...
    ros::Subscriber sub_ground_truth = n.subscribe(GROUND_TRUTH, 100, ground_truth_callback);
