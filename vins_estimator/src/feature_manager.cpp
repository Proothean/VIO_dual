...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    {
       //把当前特征点封装成一个FeaturePerFrame对象(xyz_uv_velocity)
        FeaturePerFrame f_per_fra(id_pts.second[0].second, td);
        FeaturePerFrame f_per_fra1(id_pts.second[1].second, td);
	/*
	ROS_INFO_STREAM("feature_id : "<<std::endl<< id_pts.first);
	ROS_INFO_STREAM("f_per_fra_first : "<<std::endl<< id_pts.second[0].first);
	ROS_INFO_STREAM("f_per_fra : "<<std::endl<< id_pts.second[0].second.transpose());
	ROS_INFO_STREAM("f_per_fra_first1 : "<<std::endl<< id_pts.second[1].first);
	ROS_INFO_STREAM("f_per_fra1 : "<<std::endl<< id_pts.second[1].second.transpose());
        */
	//获取当前帧的feature_id
        int feature_id = id_pts.first;
	//在滑窗的所有特征点中，看看能不能找到当前这个特征点
        auto it = find_if(feature.begin(), feature.end(), [feature_id](const FeaturePerId &it)
                          {//find的一个为此判断版本，它利用返回的布尔值的谓词判断pred，检查迭代器区间[first,last]上的每个元素，
			    //如果迭代器iter满足pred(*iter)==true;未找到元素则返回last
            return it.feature_id == feature_id;
                          });
	//没找到，特征点是个新特征点，那么加入滑窗的特征点库
        if (it == feature.end())
        {
        ...
        //找到了，该特征点已在滑窗中观测过，那么就补充上这个特征点在当前帧的数据，并且吧共视点统计数+1
    ...
    //帧数小于2帧或共视次数<20，则为关键帧
    if (frame_count < 2 || last_track_num < 20)
        return true;
    //如果不是关键帧，遍历滑窗中的每个特征点
    for (auto &it_per_id : feature)
    {	//如果当前特征点在当前帧-2以前出现过且至少在当前帧-1还在，那么该特征点为平行特征点
    ...
    if (parallax_num == 0)//条件1:平行特征点数为0
    {
        return true;
    }
    else//条件2:平均平行度小于threshold
    {
        // ROS_DEBUG("parallax_sum: %lf, parallax_num: %d", parallax_sum, parallax_num);
        // ROS_DEBUG("current parallax: %lf", parallax_sum / parallax_num * FOCAL_LENGTH);
        return parallax_sum / parallax_num >= MIN_PARALLAX;//MIN_PARALLAX = 10
    }
}
...
vector<pair<pair<Vector3d,Vector3d>,pair<Vector3d, Vector3d>>> FeatureManager::getCorresponding(int frame_count_l, int frame_count_r)
{//找到当前帧与最新一帧所有特征点在对应2帧下分别的归一化坐标，并配对，以供求出相对位姿时使用
    vector<pair<pair<Vector3d,Vector3d >,pair<Vector3d, Vector3d>>> corres;
    for (auto &it : feature)
    {
        if (it.start_frame <= frame_count_l && it.endFrame() >= frame_count_r)
        {
            Vector3d a = Vector3d::Zero(), b = Vector3d::Zero(),a1 = Vector3d::Zero(),b1 = Vector3d::Zero();
            int idx_l = frame_count_l - it.start_frame;
            int idx_r = frame_count_r - it.start_frame;

            a = it.feature_per_frame[idx_l].point;//迭代到的当前帧
	    a1= it.feature_per_frame1[idx_l].point;
            b = it.feature_per_frame[idx_r].point;//最新帧
            b1= it.feature_per_frame1[idx_r].point;
            corres.push_back(make_pair(make_pair(a,a1),make_pair(b,b1)));
        }
    }
    return corres;
}
...
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
	...
	//ROS_INFO("DEPTH= %f, %f",it_per_id.estimated_depth, it_per_id.estimated_depth1);
    }
}
...
double FeatureManager::compensatedParallax2(const FeaturePerId &it_per_id, int frame_count)
{
    ...
    /*
     ROS_INFO("u_j = %f,v_j = %f, dep_j = %f ", u_j,v_j,p_i(2));
     ROS_INFO("u_i = %f,v_i = %f, dep_i = %f ", u_i,v_i,dep_i);
     ROS_INFO("du = %f,dv = %f", du,dv);
     ROS_INFO("u_i_comp = %f,v_i_comp = %f, dep_i_comp = %f ", u_i_comp,v_i_comp,dep_i_comp);
     ROS_INFO("du_comp = %f,dv_comp = %f", du_comp,dv_comp);
     ROS_INFO("ans = %f", ans);
     */
    return ans;
}
