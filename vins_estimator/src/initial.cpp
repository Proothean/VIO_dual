...
bool Initial::initialStructure(Estimator *estimator)
{
    ...
    //TicToc t_sfm;
    //if (!checkIMUObservibility())确保IMU有足够的位移（可选）
        //return false;
    ...
    *    bool state;是否被三角化
    *    int id;
    *    vector<pair<int,Vector2d>> observation;所有观测到的特征点的 图像ID 和 特征点在这个图像帧的归一化坐标
    ...
    */
    ...
    //在滑窗（0-9）中找到第一个满足要求的帧（第l帧），它与最新一帧（frame_count=10）有足够的共视点和平行度，
    //求出这两帧之间的相对位置变化关系
    Matrix3d relative_R;
    Vector3d relative_T;
    int l;//滑窗中满足与最新帧视差关系的那一帧帧号
    if (!relativePose(estimator,relative_R, relative_T, l))
    {//这里的第L帧是从第一帧开始到滑动窗口中第一个满足与当前帧的平均视差足够大的帧l，
      //会作为 参考帧 到下面的全局sfm使用，得到Rt为当前帧到第l帧的坐标系变换Rt

        ROS_INFO("Not enough features or parallax; Move device around");
        return false;//result in estimator没有满足的帧则初始化失败
    }
    ...
    //传入frame_count+1,l,relative_R,relative_T,sfm_f这几个参数，基于L帧表示，得到Q，T，sfm_tracked_points
    //得到所有图像帧相对于参考帧的旋转四元数Q，平移向量T和特征点坐标（核心）
    //sfm_tracked_points：三角化出来的3D路标点
    if(!sfm.construct(estimator->frame_count + 1, Q, T, l,
              relative_R, relative_T,
              sfm_f, sfm_tracked_points))
    {
        ROS_DEBUG("global SFM failed!");
	//在初始化后期的第一次slidingwindow()marg掉的是old帧
...
{
    for (auto &it_per_id : estimator->f_manager.feature)//对于滑窗中出现的 所有特征点 featurePerid: feature_id
							//                            start_frame/start_frame1
                            //                            feature_per_frame/feature_per_frame1
    {
      /*
       * struct SFMFeature
{
    ...
    */
        ...
        tmp_feature.state = false;//状态：未被三角化
        tmp_feature.id = it_per_id.feature_id;//特征点id
        /*
        if(it_per_id.feature_per_frame1.size()!=it_per_id.feature_per_frame.size()){
	  ROS_INFO("wrong left and right size");
	}
	else
	  ROS_INFO("current left and right size");
	*/
        /*
        for (auto &it_per_frame : it_per_id.feature_per_frame)//对于当前特征点 在每一帧的坐标 
        {
            imu_j++;//帧标号+1
            Vector3d pts_j = it_per_frame.point;//当前特征在标号为imu_j++的帧的归一化坐标
            tmp_feature.observation.push_back(make_pair(imu_j, Eigen::Vector2d{pts_j.x(), pts_j.y()}));
	    //把当前特征在当前帧的坐标和当前帧的编号配对
        }
        */
        for (int i=0;i<it_per_id.feature_per_frame.size();i++)//对于当前特征点 在每一帧的坐标
        {
            imu_j++;//帧标号+1
            Vector3d pts_j = it_per_id.feature_per_frame[i].point;//当前特征在标号为imu_j++的帧的归一化坐标
            Vector3d pts_j1 = it_per_id.feature_per_frame1[i].point;
            tmp_feature.observation.push_back(make_pair(imu_j, Eigen::Vector2d{pts_j.x(), pts_j.y()}));
	        tmp_feature.observation1.push_back(make_pair(imu_j, Eigen::Vector2d{pts_j1.x(), pts_j1.y()}));
	    //把当前特征在当前帧的坐标和当前帧的编号配对
	     //ROS_INFO_STREAM("current left and right size"<<std::endl<<pts_j);
        }
	...
	//寻找第i帧到窗口最后一帧（当前帧）的对应特征点归一化坐标
        ...
        if (corres.size() > 20)
        {
            ...
            {//第j个对应点在第i帧和最后一帧的(x,y)
                Vector2d pts_0(corres[j].first.first(0), corres[j].first.first(1));
                Vector2d pts_1(corres[j].second.first(0), corres[j].second.first(1));
                double parallax = (pts_0 - pts_1).norm();
                sum_parallax = sum_parallax + parallax;

            }
            average_parallax = 1.0 * sum_parallax / int(corres.size());//平均视差>30
